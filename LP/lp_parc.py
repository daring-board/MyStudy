...
if __name__=='__main__':
    ...
    t1 = pulp.LpVariable('t1', 0, 1)
    t2 = pulp.LpVariable('t2', 0, 1)
    problem += (t1 + t2)
    problem += (float(u_scores[date]) + s1*W[1] + s2*W[2] + u_sign*s5*W[4] + u_sign*s6*W[5] - c_plus[date] <= t1)
    problem += (float(u_scores[date]) + s1*W[1] + s2*W[2] + u_sign*s5*W[4] + u_sign*s6*W[5] - c_plus[date] >= -t1)
    problem += (float(d_scores[date]) + s3*W[2] + s4*W[3] + d_sign*s5*W[4] + d_sign*s6*W[5] - c_minus[date] <= t2)
    problem += (float(d_scores[date]) + s3*W[2] + s4*W[3] + d_sign*s5*W[4] + d_sign*s6*W[5] - c_minus[date] >= -t2)
    print(problem)
